...
export const chat = async (req, res, next) => {
	try {
		...
		const plainChunks = document.chunks.map((c) => c.toObject());
		const relevantChunks = findRelevantChunks(plainChunks, question, 5);
		...
		res.status(200).json({
			...
			data: {
				question,
				answer,
				relevantChunks: chunkIndices, // now never contains null
