...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
	...
	return (
		<>
			...
			></div>
			<aside
				className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relative md:w-64 md:shrink-0 md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
			>
				...
				{/** Navigation */}
				<nav className="flex-1 px-1 mt-4 space-y-1.5">
					{navLinks.map((link) => (
						<NavLink
							key={link.to}
							to={link.to}
				...
				<div className="px-3 py-4 border-t border-slate-200/60">
					...
					>
						<LogOut
							size={18}
							strokeWidth={2.5}
							className="tansition-transform duration-200 group-hover:scale-110"
						/>
						Logout
					</button>
				</div>
